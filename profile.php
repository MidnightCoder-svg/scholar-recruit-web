...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($name) || empty($email)) {
        ...
        if ($result->num_rows > 0) {
            ...
            $update_sql = "UPDATE users SET name = ?, email = ?, bio = ?, skills = ?, 
                           education = ?, experience = ?, phone = ?, location = ?, website = ? 
                           WHERE id = ?";
            $update_stmt = $conn->prepare($update_sql);
            $update_stmt->bind_param("sssssssssi", $name, $email, $bio, $skills, 
                                    $education, $experience, $phone, $location, $website, $user['id']);
    ...
    if (isset($_FILES['profile_photo']) && $_FILES['profile_photo']['error'] == 0) {
        ...
        if (!in_array($_FILES['profile_photo']['type'], $allowed_types)) {
            ...
            $filename = 'user_' . $user['id'] . '_' . time() . '.' . $file_extension;
                ...
                <div class="card-body">
                    <form action="profile.php" method="post" enctype="multipart/form-data">
                            ...
                            <div class="col-md-3 text-center">
                                <div class="mb-3">
                                    <?php if (!empty($user['photo_url'])): ?>
                                        <img src="<?= htmlspecialchars($user['photo_url']) ?>" alt="Profile Photo" class="img-fluid rounded-circle mb-3" style="width: 150px; height: 150px; object-fit: cover;">
                                    <?php else: ?>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" id="skills" name="skills" rows="2" placeholder="e.g. JavaScript, PHP, MySQL, Communication"><?= htmlspecialchars($user['skills'] ?? '') ?></textarea>
                            <div class="form-text">Separate skills with commas</div>
                        </div>
                        
                        <div class="mb-3">
                            <label for="education" class="form-label">Education</label>
                            <textarea class="form-control" id="education" name="education" rows="3" placeholder="List your educational qualifications"><?= htmlspecialchars($user['education'] ?? '') ?></textarea>
                        </div>
                        
                        <div class="mb-3">
                            <label for="experience" class="form-label">Work Experience</label>
                            <textarea class="form-control" id="experience" name="experience" rows="3" placeholder="Detail your work experience"><?= htmlspecialchars($user['experience'] ?? '') ?></textarea>
                        </div>
